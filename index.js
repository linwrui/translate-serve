  ...
  try {
    ...
    if (dict) {
      ...
      for (const key in dict) {
        ...
        // 提取字典值
        const target =
          typeof dict[key] === "object" ? dict[key][toKey] : dict[key];
        if (target) {
    ...
    console.log(
      `${colors.cyan}[信息]${colors.reset} 翻译请求:`,
      JSON.stringify({ fromKey, toKey, strList, useProxy })
    );
      ...
      .map((txt) => {
        ...
        for (const key in targetDic) {
          ...
          while (transText.includes(key)) {
            console.log(
              `${colors.yellow}[替换]${colors.reset} 替换专有名词 "${key}" 为 "${targetDic[key]}"`
            );
    ...
    if (unTransList.length === 0) {
      console.log(
        `${colors.green}[成功]${colors.reset} 所有字符串已翻译，直接返回缓存结果`
      );
      return res.json({
        success: true,
    ...
    switch (engine) {
      case "google":
        console.log(
          `${colors.blue}[调用]${colors.reset} ---调用 Google 翻译 API---`
        );
        text = await googleTranslate(joinedStr, translateOptions).then(
          (res) => res.text
        );
        break;
      default:
    ...
    console.error(
      `${colors.red}[错误]${colors.reset} 错误详情:`,
      error.message,
      error.code
    );
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(
    `${colors.green}[成功]${colors.reset} 翻译服务已启动，监听端口 ${PORT}`
  );
  console.log(
    `${colors.blue}[地址]${colors.reset} 健康检查: http://localhost:${PORT}/health`
  );
  console.log(
    `${colors.blue}[地址]${colors.reset} 健康检查: http://0.0.0.0:${PORT}/health`
  );
  console.log(
    `${colors.blue}[地址]${colors.reset} 翻译接口: http://localhost:${PORT}/translate`
  );
  console.log(
    `${colors.blue}[地址]${colors.reset} 翻译接口: http://0.0.0.0:${PORT}/translate`
  );
});
